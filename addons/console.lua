...
local ClientMemoryAnalyzerClass = require(CoreGui:WaitForChild("RobloxGui"):WaitForChild("Modules")
    :WaitForChild("Stats"):WaitForChild("ClientMemoryAnalyzer"))
local ServerMemoryAnalyzerClass = require(CoreGui:WaitForChild("RobloxGui"):WaitForChild("Modules")
    :WaitForChild("Stats"):WaitForChild("ServerMemoryAnalyzer"))
local StatsUtils = require(CoreGui:WaitForChild("RobloxGui"):WaitForChild("Modules"):WaitForChild("Stats"):WaitForChild(
    "StatsUtils"))
...
function DeveloperConsole.new(screenGui, permissions, messagesAndStats)
    ...
    do -- Create top right exit button
        local exitButton, exitButtonImage = createCornerButton('Exit', 1, 0,
            'https://www.roblox.com/asset/?id=261878266', 2 / 3)
    ...
    do -- Repositioning and Resizing
        ...
        do -- Create bottom right window resize button and activate resize dragging
            local resizeButton, resizeButtonImage = createCornerButton('Resize', 1, 1,
                'https://www.roblox.com/asset/?id=261880743', 1)
    ...
    interiorFrame.Size = UDim2_new(1, -(Style.HandleHeight + Style.BorderSize * 2), 1,
        -(Style.HandleHeight + Style.BorderSize))
    ...
    do -- Options animation
        ...
        optionsButton.Position = UDim2_new(1, -(Style.GearSize + offset + Style.HandleHeight), 0,
            Style.HandleHeight + offset)
        local gear =
            Primitives.InvisibleImageLabel(optionsButton, 'Image', 'https://www.roblox.com/asset/?id=261882463')
        -- gear.ZIndex = ZINDEX + 1
        local animationToggle = devConsole:GenerateOptionButtonAnimationToggle(interiorFrame, optionsButton, gear,
            tabContainer, optionsClippingFrame, optionsFrame)
        local open = false
    ...
    do -- Options contents/filters
        ...
        do -- Search/filter/contains textbox
            ...
            textBox.Position = UDim2_new(0, label.Position.X.Offset + label.Size.X.Offset + 4, 0,
                4 + Style.CheckboxSize + 4)
            textBox.Text = ""
    ...
    do -- Console/Log tabs
        ...
        -- Wrapper for :AddTab
        local function createConsoleTab(name, text, outputMessageSync, commandLineVisible, commandInputtedCallback,
            openCallback)
                ...
                if open then
                    ...
                    if commandLineVisible then
                        if open and not commandLine then
                            ...
                            commandLine.Frame.Size = UDim2_new(1, -(Style.HandleHeight + Style.BorderSize * 2), 0,
                                Style.CommandLineHeight)
                            commandLine.Frame.Position = UDim2_new(0, Style.BorderSize, 1,
                                -(Style.CommandLineHeight + Style.BorderSize))
                            commandLine.CommandInputted:connect(commandInputtedCallback)
                        end
                    end

                    window.Size = commandLineVisible and UDim2_new(1, 0, 1, -(Style.HandleHeight)) or
                                      UDim2_new(1, 0, 1, 0)
        ...
        if permissions.MayViewClientLog then
            local tab = createConsoleTab('ClientLog', "Client Log", devConsole.MessagesAndStats.OutputMessageSyncLocal,
                true, function(text)
                    if #text <= 1 then
                        return
                    end
...
function Methods.CreateScrollbar(devConsole, rotation)
    ...
    local buttonEffectFunction = devConsole:CreateButtonEffectFunction(bar, nil, bar.BackgroundColor3,
...
function Methods.GenerateOptionButtonAnimationToggle(devConsole, interior, button, gear, tabContainer,
    optionsClippingFrame, optionsFrame)

    local tabContainerSize0 = tabContainer.Size
    local tabContainerSize1 = UDim2_new(tabContainerSize0.X.Scale,
        tabContainerSize0.X.Offset + (Style.GearSize + 2) + Style.BorderSize, tabContainerSize0.Y.Scale,
    ...
    local interiorSize1 = UDim2_new(interiorSize0.X.Scale, interiorSize0.X.Offset, interiorSize0.Y.Scale,
        interiorSize0.Y.Offset - Style.OptionAreaHeight)
    local interiorPosition0 = interior.Position
    local interiorPosition1 = UDim2_new(interiorPosition0.X.Scale, interiorPosition0.X.Offset,
    ...
    return function(open)
        if open then
            ...
            optionsClippingFrame:TweenSizeAndPosition(UDim2_new(1, 0, 0, Style.OptionAreaHeight),
                UDim2_new(0, 0, 0, -Style.OptionAreaHeight), 'Out', 'Sine', length, true)
            optionsFrame:TweenPosition(UDim2_new(0, 0, 0, 0), -- -Style.OptionAreaHeight),
            ...
            optionsClippingFrame:TweenSizeAndPosition(UDim2_new(1, 0, 0, 0), UDim2_new(0, 0, 0, 0), 'Out', 'Sine',
                length, true)
...
function Methods.ConnectButtonDragging(devConsole, button, dragCallback, mouseInteractCallback)
    ...
    local function startDragging(startP)
        ...
        listeners[#listeners + 1] = UserInputService.InputChanged:connect(function(input)
            ...
            if not (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType ==
                Enum.UserInputType.Touch) then -- added in a touch check
                return
            end
...
do
    local permissions
    local permissionsLoading = false;
    function DeveloperConsole.GetPermissions()
        ...
        local success, result = pcall(function()
            local url = string.format("/users/%d/canmanage/%d", game:GetService("Players").LocalPlayer.UserId,
                game.PlaceId)
            return game:GetService('HttpRbxApiService'):GetAsync(url, Enum.ThrottlingPriority.Default,
                Enum.HttpRequestType.Default, true)
        end)
        ...
        pcall(function()
            permissions.ServerCodeExecutionEnabled = permissions.IsCreator and
                                                         (not settings():GetFFlag("DebugDisableLogServiceExecuteScript"))
        end)
...
do
    ...
    function DeveloperConsole.GetMessagesAndStats(permissions)
        ...
        local warningsToFilter = {"ClassDescriptor failed to learn", "EventDescriptor failed to learn",
...
local function onDevConsoleVisibilityChanged(isVisible)
    ...
    if isVisible then
        ...
        ContextActionService:BindCoreAction(blockMenuActionName, function()
        end, false, Enum.KeyCode.ButtonStart)
