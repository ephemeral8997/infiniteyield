            ...
            if cmd then
                ...
                if store then
                    if speaker == Players.LocalPlayer then
                        if cmdHistory[1] ~= rawCmdStr and rawCmdStr:sub(1, 11) ~= "lastcommand" and rawCmdStr:sub(1, 7) ~=
                            "lastcmd" then
    ...
    if httprequest then
        ...
        if body and body.data then
            for i, v in next, body.data do
                if type(v) == "table" and tonumber(v.playing) and tonumber(v.maxPlayers) and v.playing < v.maxPlayers and
                    v.id ~= JobId then
...
function createESP(plr)
    ...
    local function setupCharacter(char)
        if not char or not folder.Parent then
            return
        end
        ...
        ESPConnections[plr] = heartbeatConnection
    end
    ...
    local characterConnection = plr.CharacterAdded:Connect(function(char)
        if ESPenabled then
            task.wait()
            setupCharacter(char)
        end
...
function enableESP()
    ...
    notify("ESP", "✅ ESP enabled with full player coverage")
end
...
addcmd("noesp", {}, function(args, speaker)
    if ESPenabled then
        disableESP()
    else
        notify("ESP", "ESP is already disabled")
    end
end)
...
addcmd("nolimbs", {"rlimbs"}, function(args, speaker)
    if r15(speaker) then
        for i, v in pairs(speaker.Character:GetChildren()) do
            if v:IsA("BasePart") and v.Name == "RightUpperLeg" or v.Name == "LeftUpperLeg" or v.Name == "RightUpperArm" or
        ...
        for i, v in pairs(speaker.Character:GetChildren()) do
            if v:IsA("BasePart") and v.Name == "Right Leg" or v.Name == "Left Leg" or v.Name == "Right Arm" or v.Name ==
                "Left Arm" then
                v:Destroy()
            end
        end
...
addcmd("oldconsole", {}, function(args, speaker)
    ...
    local _, str = pcall(function()
        return game:HttpGet(
            "https://raw.githubusercontent.com/ephemeral8997/infiniteyield/refs/heads/master/addons/console.lua", true)
    end)
...
addcmd("headsit", {}, function(args, speaker)
    ...
    for i, v in pairs(players) do
        ...
        headSit = RunService.Heartbeat:Connect(function()
            if Players:FindFirstChild(Players[v].Name) and Players[v].Character ~= nil and getRoot(Players[v].Character) and
                getRoot(speaker.Character) and speaker.Character:FindFirstChildOfClass("Humanoid").Sit == true then
                getRoot(speaker.Character).CFrame = getRoot(Players[v].Character).CFrame *
            ...
            while Tool and speaker.Character and v.Character and Tool.Parent == speaker.Character do
                ...
                for _, v1 in ipairs(v.Character.GetChildren(v.Character)) do
                    v1 = ((v1.IsA(v1, "BasePart") and
                             firetouchinterest(Handle, v1, 1, (RS.Wait(RS) and nil) or firetouchinterest(Handle, v1, 0)) and
                             nil) or v1) or v1
                end
            end
...
addcmd("stareat", {"stare"}, function(args, speaker)
    ...
    for i, v in pairs(players) do
        ...
        local function stareFunc()
            if Players.LocalPlayer.Character.PrimaryPart and Players:FindFirstChild(v) and Players[v].Character ~= nil and
                Players[v].Character:FindFirstChild("HumanoidRootPart") then
    ...
    for i, v in pairs(players) do
        ...
        for _, part in pairs(workspace:GetDescendants()) do
            if Players[v].Character:FindFirstChild("Head") and part:IsA("BasePart" or "UnionOperation" or "Model") and
                part.Anchored == false and not part:IsDescendantOf(speaker.Character) and part.Name == "Torso" == false and
